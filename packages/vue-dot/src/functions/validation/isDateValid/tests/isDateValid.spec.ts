...
describe('isDateValid', () => {
	...
	it('returns an error when it\'s february (not leap year) and the day doesn\'t match the month', () => {
		const date = '29/02/2019'; // Not a leap year, only 28 days

		expect(typeof isDateValid(date)).toBe('string');
	});

	it('returns an error when it\'s february (leap year) and the day doesn\'t match the month', () => {
