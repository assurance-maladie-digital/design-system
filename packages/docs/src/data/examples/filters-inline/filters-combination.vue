<template>
	<div>
		<v-col>
			<FiltersSideBar v-model="filters" align="center">
				<template #name="{ on, attrs }">
					<VTextField
						v-bind="attrs"
						label="Nom"
						outlined
				...
				<template #period="{ on, attrs }">
					<PeriodField v-bind="attrs" outlined v-on="on" />
				</template>

				<template #profession="{ on, attrs }">
					<SearchListField
						v-bind="attrs"
...
@Component
export default class FiltersCombination extends Vue {
	filters = [
		{
			name: "name",
			title: "Identité",
