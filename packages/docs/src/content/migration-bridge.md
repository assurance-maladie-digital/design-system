...
1- Téléchargez la stack de migration `starter-kit-vue-bridge`

2- Copier le contenu dossier `src` de votre projet VueJS 2 dans le dossier `src` du projet `starter-kit-vue-bridge`

3- Ajouter les dépendances à jour de votre projet VueJS 2 dans le nouveau projet `starter-kit-vue-bridge`

4 - Dans le fichier `nuxt.config.ts`, ajouter le lien vers votre fichier de style global.
...
## 5 - Migration du routeur

5.1 - Mettre à jour le tableau de déclaration des routes vue-router avec la version 4.
...
5.2 - Dans le fichier ‘app/router.options.ts’, mettre à jour le path de votre fichier de configuration de route si besoin.
...
5.3 - Dans la version 4 de vue-router, certains types ont été remplacés. Il faut effectuer les changements de types dans le projet.
...
## 6 - Animations des pages

Si le projet utise des transitions de page,

6.1 - Retirer le composent '<transition>' dans le fichier 'app.vue'

6.2 - Configurer une transition de page dans le fichier 'nuxt.config.ts'
...
6.3 - Définir le style de la transition dans un fichier de style global.
...
## 7 - Utilisation du script [vue-class-migrator](https://github.com/getyourguide/vue-class-migrator)
...
7.1 - Supprimer tous les décorateurs personnalisés, ils ne seront pas reconnus par vue-class-migrator et causeront l'échec de la migration du composant.

7.2 - Executer la commande `npx vue-class-migrator -d .` à la racine de votre projet.

7.3 - Consulter les logs dans le terminal pour repérer les éventuels composants causant des erreurs et n'ayant pas pu être migré automatiquement.

7.4 - Si vous avez déclaré vos props en tant que mixins dans vos composants en utilisant le helper 'mixin' de vue-class-componant, modifier l'import de la mixin dans le composant.
...
7.5 - Implémenter les fonctionnalités  qui utilisaient des décorateurs retiré à l'étape 1 en utilisant l'option API, des mixins ou des composables.
...
## 9 - Migration des composants Vuetify
...
## 10 - Migration des tests unitaires et de composants
...
L'API de Vitest est similaire a celle de Jest, pour faire des mock il faut désormais utiliser `vi` au lieu de `jest`.
