...
describe("FormFieldList", () => {
	it("renders correctly", () => {
		const wrapper = mount({
			template: '<VApp><FormFieldList v-model="addressFields" listTitle="title" /></VApp>',
			data() {
				return {
					addressFields,
	...
	it("emit a update:modelValue event when a field is updated", async () => {
		...
		expect(FormFieldListComponent.emitted()).toHaveProperty("update:modelValue");
	});

	it("emit a refresh event when a field is updated", async () => {
		const wrapper = mount({
			template: '<VApp><FormFieldList v-model="addressFields" /></VApp>',
			data() {
				return {
					addressFields: {
						...addressFields,
