...
export default defineComponent({
	...
	computed: {
		doOtherChoiceHaveTrigger(): boolean {
			return (
				(
					Array.isArray(this.field.other) && this.field.other.length > 0
				) || (
					(this.field.other as OtherField)?.selectedChoice !== undefined &&
				...
				return this.field.other.some((other) => {
					return (
						...
						(
							...
							this.choiceValue.value.includes(other.selectedChoice)
						)
					);
				});
			}
			...
			if (Array.isArray(this.field.other)){
				if (Array.isArray(this.choiceValue.value)) {
					return this.field.other.find(
						other => (this.choiceValue.value as FieldItemValue[]).includes(
							other.selectedChoice
						)
					)
				} else {
					return this.field.other.find(element => element.selectedChoice === this.choiceValue.value);
				}
			} else {
			...
			<VExpandTransition hide-on-leave>
				<div v-if="showOtherTextareaField" class="vd-other-field mt-4">
					...
					<VTextarea
						...
						variant="outlined"
						class="vd-form-input"
						:color="field.fieldOptions?.color as string ?? 'primary'"
