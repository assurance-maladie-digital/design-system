...
describe('choiceField', () => {
  ...
  it('emit an updateValue event when the text area is used', async () => {
    ...
    expect(
      (choiceField.emitted('update:modelValue')?.[0]?.[0] as any)?.value
        ?.other,
    ).toBe('test');
  });
  ...
  it('displays error messages when an object is passed', async () => {
    ...
    expect(wrapper.find('.vd-select-btn-field .v-messages').text()).toBe(
      'Erreur 1',
    );
    expect(
      wrapper.find('.vd-other-field .v-messages__message').text(),
    ).toBe('Erreur 2');
  });
  ...
  it('renders the other field when the select is of type multiple and there is many other field', () => {
    ...
    expect(wrapper.html()).toContain(
      'Merci d\'expliquer les raisons de votre réponse ?',
    );
  });
});
