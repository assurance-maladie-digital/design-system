...
describe('scrollBehavior', () => {
	it('returns the default scroll postion', () => {
		expect(scrollBehavior(defaultRoute, defaultRoute)).toStrictEqual(
			defaultPosition
		)
	})
	...
	it('returns the selector custom hash instead of saved position', () => {
		...
		expect(
			scrollBehavior(hashRoute, defaultRoute, savedPosition)
		).toStrictEqual({ selector: customHashSelector })
	})

	it('returns the saved position', () => {
		expect(scrollBehavior(defaultRoute, defaultRoute, savedPosition)).toBe(
			savedPosition
		)
	})
})
