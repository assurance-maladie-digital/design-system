  ...
  computed: {
    ...
    itemValue(): string {
      if (typeof this.value === 'number') {
        return Number.isNaN(this.value) ? this.placeholder : this.value.toString();
      }
...
<template>
  <li class="vd-data-list-item d-flex flex-wrap">
    <slot name="icon">
      <VIcon v-if="icon" v-bind="$options.icon">
        {{ icon }}
      </VIcon>
    </slot>
    ...
    <div class="vd-data-list-item-content">
      <div :class="{ 'vd-row': row }">
        ...
        <div class="vd-data-list-item-value">
          <slot name="value" v-bind="{ itemValue }">
            <VChip v-if="chip" v-bind="$options.chip">
              {{ itemValue }}
            </VChip>
