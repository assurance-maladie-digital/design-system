...
export default defineComponent({
	...
	methods: {
		calendarEvents(date: string): Events {
			const userEvents = this.options.datePicker
				?.events as EventsFunction | null;

			if (userEvents) {
				return typeof userEvents === "function"
					? userEvents(date)
					: userEvents;
			}
			...
			if (this.showWeekends) {
				const weekEndColor = this.$vuetify.theme.dark
					? "grey darken-1"
					: "grey lighten-1";

				return isWeekend(date)
					? `vd-custom-event ${weekEndColor}`
					: false;
			}
