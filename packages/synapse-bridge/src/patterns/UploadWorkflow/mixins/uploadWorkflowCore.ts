...
import type { FileListItem, SelectItem, SelectedFile } from '../types';
...
export default defineComponent({
  ...
  methods: {
    ...
    async previewFile(file: FileListItem | File): Promise<void> {
      if (!this.showViewBtn) {
        consola.warn(
          'Using `vuetify-options` for this functionality is deprecated since v2.16.0,'
          + ' use the `show-view-btn` prop instead.',
        );
