...
function createTestStore(notificationState?: { notification: any }) {
	return createStore({
		...notificationState,
		modules: {
			notification,
...
// eslint-disable-next-line ts/no-explicit-any
function createNotificationBar(store: Store<any>, options?: any) {
...
describe('NotificationBar', () => {
	...
	describe('Computed properties', () => {
		it('returns true if display name is xs, otherwise false', () => {
			...
			expect(wrapperWithXsDisplay.vm.mobileVersion).toBeTruthy();

			const storeWithNonXsDisplay = createTestStore({
				notification: null,
			});
			const wrapperWithNonXsDisplay = createNotificationBar(storeWithNonXsDisplay);

			wrapperWithNonXsDisplay.vm.$vuetify.display.name = 'sm'; // Assuming display name is not xs
			expect(wrapperWithNonXsDisplay.vm.mobileVersion).toBeFalsy();
		});
		...
		it('computes isLongText correctly', async() => {
			...
			expect(wrapper.vm.isLongText).toBeTruthy();
		});
		...
		it('computes isLongText correctly for short text', async() => {
			...
			expect(wrapper.vm.isLongText).toBeFalsy();
		});
		...
		it('computes hasLongContent correctly', () => {
			...
			expect(wrapper.vm.hasLongContent).toBeFalsy();
		});
		...
		it('computes hasLongContent correctly when there is an action slot', () => {
			...
			expect(wrapper.vm.hasLongContent).toBeTruthy();
		});
		...
		it('computes smallCloseBtn correctly', () => {
			...
			expect(wrapper.vm.smallCloseBtn).toBeFalsy();
		});
		...
		it('returns true when mobileVersion is true and hasLongContent is false', async() => {
			...
			expect(wrapper.vm.smallCloseBtn).toBeTruthy();
		});
