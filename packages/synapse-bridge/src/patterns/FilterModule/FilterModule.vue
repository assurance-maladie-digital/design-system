...
export default defineComponent({
  ...
  computed: {
    filtersCount(): number {
      return this.appliedFilters.filter(item => item.value !== null)
        .length;
    },
  },
  methods: {
    ...
    emitFilterListEvent(): void {
      this.$emit('filterList', this.appliedFilters);
    },
    resetFilters(): void {
      this.appliedFilters = [];
      this.emitFilterListEvent();
...
<template>
  <div class="vd-filter-module">
    ...
    >
      <VForm
        class="vd-filter-module-dialog-content d-flex flex-column"
