...
describe('FilterModule', () => {
	...
	it('remove the filter and emit a "filter-list" event', async () => {
		...
		expect((wrapper.emitted('filter-list')![0]![0] as any)![0]!.value).toBe(
			'new filter'
		)
	...
	it('remove the filter and emit a "filter-list" event with the other filter in', async () => {
		...
		await wrapper
			...
			.find('.vd-filter-module-dialog-actions button')
			.trigger('click')
		expect((wrapper.emitted('filter-list')![1]![0] as any)![1]!.value).toBe(
			'new filter 2'
		)
	...
	it('update the filter and emit a "filter-list" event', async () => {
		...
		await wrapper
			.findComponent(VCard)
			.find('input')
			.setValue('new filter 2')
		await wrapper
			.findComponent(VCard)
			.find('.vd-filter-module-dialog-actions button')
			.trigger('click')

		expect((wrapper.emitted('filter-list')![2]![0] as any)![0]!.value).toBe(
			'new filter 2'
		)
		expect((wrapper.emitted('filter-list')![2]![0] as any)![1]!.value).toBe(
			'lorem ipsum'
		)
	})
	...
	it('reset all the filters', async () => {
		...
		await wrapper
			...
			.trigger('click')
		await wrapper
			.find('.vd-filter-module button .v-icon[role="button"]')
			.trigger('click')
