...
export default defineComponent({
	...
	props: {
		theme: {
			...
			validator: (value: ThemeEnum) => propValidator('theme', THEME_ENUM_VALUES, value),
		},
		serviceTitle: {
			type: String,
			default: undefined,
	...
	computed: {
		...
		spacingClass(): string {
			return (
				this.sticky &&
					this.scrolled ?
						(this.isMobileVersion ? 'px-4 py-1' : 'px-14 py-1') :
						(this.isMobileVersion ? 'pa-4' : 'px-14 py-7')
			);
		},
		contentSheetHeight(): number {
			return (
				this.isMiniVersion ?
						(this.isMobileVersion ? 52 : 72) :
						(this.isMobileVersion ? 72 : 120)
			);
		},
		fullHeight(): number {
